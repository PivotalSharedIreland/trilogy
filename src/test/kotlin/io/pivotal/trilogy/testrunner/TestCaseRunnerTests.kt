...
class TestCaseRunnerTests : Spek({
    ...
    it("should run successful test case") {
        ...
        expect(true) { TestCaseRunner(testSubjectCaller, assertionExecutor).run(trilogyTestCase).didPass }
    }

    it("should run the failing test case") {
        val trilogyTestCase = Fixtures.getTestCase("should_fail")

        expect(false) { TestCaseRunner(testSubjectCaller, assertionExecutor).run(trilogyTestCase).didPass }
    }
    ...
    context("test case with one passing and one failing test") {
        ...
        it("should report the number of failing tests") {
            expect(1) { TestCaseRunner(testSubjectCaller, assertionExecutor).run(trilogyTestCase).failed }
        }

        it("should report the number of passing tests") {
            expect(1) { TestCaseRunner(testSubjectCaller, assertionExecutor).run(trilogyTestCase).passed }
        }
    }
